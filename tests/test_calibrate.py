...
def test_floor_is_exact_not_close():
...
def test_bored_middle_the_fixed_floor_sleeps_through_it():
...
def test_no_alarm_storm_on_the_ordinary_chop():
    cal = CalibratedFloor(k=8, slack=Q16(5, 2))
    for _ in range(8):
        cal.update(Q16(1, 100))
...
def test_floor_relaxes_after_a_spike_no_permanent_deafness():
    cal = CalibratedFloor(k=8, slack=Q16(5, 2))
    for _ in range(8):
        cal.update(Q16(1, 100))
    before = cal.floor()
...
def test_inexact_k_and_float_slack_refused():
